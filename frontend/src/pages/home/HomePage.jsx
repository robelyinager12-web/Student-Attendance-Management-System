...
function StatCard({ label, value, icon, color, subtitle, onClick }) {
  const colors = {
    indigo: 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 border-indigo-100 dark:border-indigo-800',
    green:  'bg-green-50  dark:bg-green-900/20  text-green-600  border-green-100  dark:border-green-800',
    red:    'bg-red-50    dark:bg-red-900/20    text-red-600    border-red-100    dark:border-red-800',
    yellow: 'bg-yellow-50 dark:bg-yellow-900/20 text-yellow-600 border-yellow-100 dark:border-yellow-800',
    ...
    >
      <div className="flex items-start justify-between">
        ...
        <div className={`w-11 h-11 rounded-xl flex items-center justify-center
          text-xl ${c.split(' ').slice(0, 2).join(' ')}`}>
          {icon}
        </div>
      </div>
    </div>
...
const PIE_COLORS = ['#4F46E5','#06B6D4','#34d399','#f87171','#f59e0b','#a78bfa'];
...
function AdminHome({ user }) {
  ...
  const qColor = {
    ...
    red:    'bg-red-50    dark:bg-red-900/20    text-red-600    hover:bg-red-100',
  };
    ...
    <div className="space-y-6">
          ...
          {quickActions.map((a) => (
            <button key={a.label} onClick={() => navigate(a.path)}
              className={`flex flex-col items-center gap-2 p-4 rounded-xl
                transition-colors text-sm font-semibold ${qColor[a.color]}`}>
              {a.icon}{a.label}
            </button>
          ))}
        </div>
          ...
          {weeklyData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={weeklyData}>
                ...
                <Tooltip />
                <Area type="monotone" dataKey="PRESENT" stroke="#4F46E5" fill="url(#pg)" strokeWidth={2} name="Present" />
                <Area type="monotone" dataKey="ABSENT"  stroke="#f87171" fill="url(#ag)" strokeWidth={2} name="Absent" />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
          ...
          {deptData.filter(d => d.studentCount > 0).length > 0 ? (
            <>
              <ResponsiveContainer width="100%" height={160}>
                <PieChart>
                  <Pie data={deptData.filter(d => d.studentCount > 0)} dataKey="studentCount"
                    nameKey="department" cx="50%" cy="50%" outerRadius={70} innerRadius={40}>
                ...
                {deptData.filter(d => d.studentCount > 0).slice(0, 4).map((d, i) => (
                  <div key={d.department} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-600 dark:text-gray-300 truncate max-w-[110px]">
                        {d.department}
                      </span>
                    </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-5">
          ...
          {batchStats.length === 0 ? (
            <p className="text-sm text-gray-400 py-6 text-center">
              No batches yet. <button onClick={() => navigate('/batches')}
                className="text-indigo-600 underline">Add batches</button>
            </p>
          ) : (
            <div className="space-y-2">
              {batchStats.slice(0, 5).map((b) => (
                <div key={b.id} className="flex items-center justify-between p-3
                  ...
                  <span className="text-sm font-bold text-indigo-600">{b.studentCount} students</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-5">
          ...
          {recentAct.length === 0 ? (
            <p className="text-sm text-gray-400 py-6 text-center">
              No attendance records yet. <button onClick={() => navigate('/attendance')}
                className="text-indigo-600 underline">Take attendance</button>
            </p>
          ) : (
            <div className="space-y-2">
              {recentAct.slice(0, 5).map((a) => (
                <div key={a.id} className="flex items-center justify-between p-2
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-700 dark:text-gray-200">{a.studentName}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {lowAttend.slice(0, 6).map((s) => (
              <div key={s.id} className="flex items-center justify-between p-3
                ...
                <span className="text-lg font-bold text-red-600">{s.attendancePercentage}%</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <StatCard label="Today's Records"    value={stats?.attendanceTakenToday ?? 0} icon={<MdCheckCircle />} color="green"  subtitle="Submitted today" />
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: 'Take Attendance',    path: '/attendance',         icon: <MdCheckCircle size={22} />, bg: 'bg-green-50  dark:bg-green-900/20  text-green-600  hover:bg-green-100' },
            { label: 'Attendance History', path: '/attendance/history', icon: <MdHistory size={22} />,     bg: 'bg-blue-50   dark:bg-blue-900/20   text-blue-600   hover:bg-blue-100' },
            { label: 'My Students',        path: '/students',           icon: <MdPeople size={22} />,      bg: 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 hover:bg-indigo-100' },
            { label: 'View Reports',       path: '/reports',            icon: <MdBarChart size={22} />,    bg: 'bg-purple-50 dark:bg-purple-900/20 text-purple-600 hover:bg-purple-100' },
          ].map((a) => (
            <button key={a.label} onClick={() => navigate(a.path)}
              className={`flex flex-col items-center gap-2 p-4 rounded-xl
                transition-colors text-sm font-semibold ${a.bg}`}>
              {a.icon}{a.label}
            </button>
          ))}
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {courses.map((a) => (
              <div key={a.id} className="p-4 bg-gradient-to-br from-indigo-50 to-blue-50
                ...
                <p className="font-semibold text-gray-800 dark:text-white text-sm">{a.Course?.name}</p>
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-blue-100 text-sm mt-0.5">
              {[stats?.department, stats?.batch, stats?.academicYear, stats?.semester].filter(Boolean).join(' · ')}
            </p>
            {stats?.section && (
              <p className="text-blue-200 text-xs mt-1">Section: {stats.section}</p>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border
          ...
          <svg width="120" height="120" viewBox="0 0 100 100">
            ...
            />
            <text x="50" y="46" textAnchor="middle" fontSize="16" fontWeight="800" fill={pctColor}>
              {pct.toFixed(0)}%
            </text>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border
          ...
          <div className="space-y-3">
            {[
              { label: 'View Attendance History', icon: <MdHistory size={18} />,      path: '/attendance/history', bg: 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600' },
              { label: 'My Notifications',        icon: <MdNotifications size={18} />, path: '/notifications',      bg: 'bg-blue-50 dark:bg-blue-900/20 text-blue-600' },
              { label: 'My Profile',              icon: <MdPeople size={18} />,        path: '/profile',            bg: 'bg-green-50 dark:bg-green-900/20 text-green-600' },
            ].map((a) => (
              <button key={a.label} onClick={() => navigate(a.path)}
                className={`flex items-center gap-3 w-full p-3 rounded-xl
                  text-sm font-medium transition-opacity hover:opacity-80 ${a.bg}`}>
                {a.icon}
                {a.label}
      ...
      {stats?.calendar?.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl border
          ...
          <SectionHeader title="Recent Attendance Records" linkLabel="View all" linkTo="/attendance/history" navigate={navigate} />
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {stats.calendar.slice(0, 8).map((r, i) => (
                  <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                    <td className="py-2.5 text-gray-700 dark:text-gray-300">{formatDate(r.date)}</td>
