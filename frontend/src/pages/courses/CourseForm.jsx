...
function CourseForm({ initialData, onSuccess, onCancel }) {
  ...
  const onSubmit = async (data) => {
    try {
      if (initialData) {
        await courseService.update(initialData.id, data);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      {[
        ['Course Name', 'name', { required: 'Name is required' }],
        ['Course Code', 'code', { required: 'Code is required' }],
